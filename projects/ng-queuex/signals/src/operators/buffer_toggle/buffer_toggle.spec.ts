...
describe('Testing bufferToggle function.', () => {
  ...
  it('Should project output signal correctly', () => {
    ...
    const scope = createTestCleanupScope();
    const outputSource = scope.run(() => bufferToggle<string | undefined, any>(openings, () => closingNotifier)(inputSource));
  ...
  it('Closing notifier should run in child cleanup scope.', () => {
    ...
    const outputSource = scope.run(() => bufferToggle<string | undefined, any>(openings, () => {
  ...
  it('Should run cleanup logic if cleanup scope gets clean immediate in closing selector.', () => {
    ...
    const closingNotifier = signal(0);
    const scope = createTestCleanupScope();
    scope.run(() => bufferToggle<string | undefined, any>(openings, () => {
      CleanupScope.assertCurrent().cleanup();
  ...
  it('Should run cleanup logic if cleanup scope gets clean immediate in closing notifier.', () => {
    ...
    const closingNotifier = signal(0);
    const scope = createTestCleanupScope();
    scope.run(() => bufferToggle<string | undefined, any>(openings, () => computed(() => {
      CleanupScope.assertCurrent().cleanup();
      CleanupScope.assertCurrent().add(() => log.push('A'));
      return closingNotifier();
    }))(inputSource));
  ...
  it('Should clean host cleanup scope if cleanup scope gets clean immediately during signal read.', () => {
    ...
    const outputSource = scope.run(() => bufferToggle<string | undefined, any>(computed(() => {
      CleanupScope.assertCurrent().cleanup();
      return openings();
  ...
  it('Should emit open values.', () => {
    ...
    const outputSource = scope.run(() => bufferToggle<number | undefined, string | undefined>(openings, (openValue) => {
      log.push(openValue);
      return closingNotifier;
    })(inputSource));
  ...
  it('Should child cleanup scope be destroyed after cleanup.', () => {
    ...
    const openings = signal<string | undefined>(undefined);
    const scope = createTestCleanupScope();
    scope.run(() => bufferToggle(openings, () => signal(undefined))(inputSource));
