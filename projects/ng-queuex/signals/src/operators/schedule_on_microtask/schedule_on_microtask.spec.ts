...
import { scheduleOnMicrotask } from "./schedule_on_microtask";
...
describe('Testing scheduleOnMicrotask() function.', () => {
  ...
  it('Should throw error if it is used outside cleanup scope.', () => {
    expect(() => scheduleOnMicrotask()(signal(undefined))).toThrowError(
      'scheduleOnMicrotask(): Current stack frame is not within cleanup scope.'
    );
  });
  ...
  it('Should throw error if it is used inside reactive context.', () => {
    ...
    scope.run(() => expect(() => runInReactiveContext(() => scheduleOnMicrotask()(signal(undefined)))).toThrowError());
  });
  ...
  it('Should project output signal correctly.', fakeAsync(() => {
    ...
    const outputSource = scope.run(() => scheduleOnMicrotask<string | undefined>()(inputSource));
